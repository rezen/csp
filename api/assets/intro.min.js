...
function triggerTutorial() {
    ...
    guide.setOption("overlayOpacity", 1);

    guide.addStep({
        element: document.querySelectorAll('.csp-directive')[0],
        intro: "There are rows for the various CSP directives that you can play with & alter. Check <a href='https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy'>MDN</a> for a detailed list of available directives.",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('.csp-directive select')[0],
        intro: "The directive can be changed by selecting a directive option in this list",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('.csp-directive textarea')[0],
        intro: "Edit this textarea, specifying the valid sources for this scope such as <code>http://*.example.com</code>, <code>mail.example.com:443</code>, <code>'self'</code>, <code>'none'</code>, <code>'nonce-{{nonce}}'</code> ",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('#csp-form button[type=submit]')[0],
        intro: "Click update to reload the page with your modified CSP",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('#csp-examples tr')[1],
        intro: "This is a row that has an element that you need to control with a CSP",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('#csp-examples tr .col-state')[0],
        intro: "This column indicates if the resource was allowed or blocked",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('#csp-examples tr .col-goal')[0],
        intro: "This column indicates the goal, whether we want to allow or block this element",
        position: 'right'
    }).addStep({
        element: document.querySelectorAll('#csp-examples tr .col-goal + td')[0],
        intro: "This column indicates whether the goal was accomplished",
        position: 'right'
    });
    guide.onafterchange(function(targetElement) {
        if (targetElement.tagName === 'TR') {
            document.getElementById('csp-examples').classList.add("in-tutorial");
        }
    }).oncomplete(function() {
        if (document.cookie.indexOf('didTutorial') !== -1 ){
            return;
        }
        alert();
        document.cookie = 'didTutorial=1; path=/';
    });
    guide.refresh();
    guide.start();
}
